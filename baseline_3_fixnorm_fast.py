...
class LitSystem(pl.LightningModule):
    ...
    def apply_qtransform(self, waves):
        bs = waves.shape[0]
        waves = self.wave_transform(waves.view(bs*3,-1))
        _, h, w = waves.shape
        #batch_wave = batch_wave.view(bs,3,h,w)
        waves = waves.view(bs,1,3*h,w)
        waves = (waves-0.013205823003608798)/0.037445450696502146
        return waves
