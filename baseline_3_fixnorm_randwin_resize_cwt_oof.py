...
class G2NetDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.train:
            label1 = torch.tensor([self.pred[idx]]).float()
        else:
        ...
        if self.train:
            if torch.rand(1) < 0.50:
                ...
                label2 = torch.tensor([self.pred[indx]]).float()
...
class SETIDataModule(pl.LightningDataModule):
    ...
    def setup(self, stage=None):
        if stage == 'fit':
            ...
            oof = pd.read_csv(os.path.join("oof8824.csv"))
            #oof['dir'] = os.path.join(self.conf.data_dir, "train")
            df = pd.merge(df,oof)
            ...
            if self.conf.pseudo is not None:
                ...
                pseudo_df['pred'] = pseudo_df['target']
