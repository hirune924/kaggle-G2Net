...
class CustomNet(nn.Module):
    def __init__(self, base_model):
        ...
        self.qtransform = CQT1992v2(sr=2048, fmin=20, fmax=1024, hop_length=8, bins_per_octave=8, window='flattop', trainable=True)
    ...
    def forward(self, x):
        ...
        x = self.qtransform(x)
        shape = x.shape
        x = x.view(batch, chans, shape[1], shape[2])
...
class LitSystem(pl.LightningModule):
    def __init__(self, conf):
        ...
        self.model = CustomNet(base_model=timm.create_model(model_name=self.hparams.model_name, num_classes=1, pretrained=True, in_chans=3,
